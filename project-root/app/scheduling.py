...
def add_availability( trainer_id: int, start_date: date, end_date: date, start_time: time, end_time: time):
...
def get_available_rooms(start_date: date, end_date: date, start_time: time, end_time: time):
    if start_time >= end_time:
        raise ValueError("Start time must be earlier than end time.")
    ...
    with SessionLocal() as db:
        ...
        # find rooms that are NOT in conflict with any existing availability
        conflicted_rooms = (
        ...
        available_rooms = (
            db.query(Rooms)
            .filter(Rooms.id.notin_(conflicted_rooms))
            .all()
        )
